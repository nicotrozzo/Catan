...
player1Viewer::player1Viewer(catanPlayerModel * player1)
{
	...
	if ((fontForVictoryPoints = al_load_ttf_font("graficoCatan\\font\\scribish.ttf", 55 * PROP_Y, 0)) != NULL)
	{
		...
		{
			if ((fontForResources = al_load_ttf_font("graficoCatan\\font\\scribish.ttf", 31 * PROP_Y, 0)) != NULL)
			{
...
void player1Viewer::viewVictoryPoints()
{
	...
	al_draw_text(fontForVictoryPoints, al_color_name("black"), 148 * PROP_X, 575 * PROP_Y, 0, points.c_str());
}
...
void player1Viewer::viewNameIs()
{
	...
	if (name.size() == 0)
	{
		al_draw_text(fontForName, al_color_name("black"), 27 * PROP_X, 28 * PROP_Y, 0, "Player1");
	}
	else
	{
		al_draw_text(fontForName, al_color_name("black"), 27 * PROP_X, 28 * PROP_Y, 0, (name.substr(0,6)).c_str());
	}
}
...
void player1Viewer::viewResources()
{
	...
	al_draw_text(fontForResources, al_color_name("black"), 15 * PROP_X, 150 * PROP_Y, 0, to_string(ore).c_str());
	al_draw_text(fontForResources, al_color_name("black"), 115 * PROP_X, 150 * PROP_Y, 0, to_string(wheat).c_str());
	al_draw_text(fontForResources, al_color_name("black"), 215 * PROP_X, 150 * PROP_Y, 0, to_string(brick).c_str());
	al_draw_text(fontForResources, al_color_name("black"), 56 * PROP_X, 275 * PROP_Y, 0, to_string(wood).c_str());
	al_draw_text(fontForResources, al_color_name("black"), 159 * PROP_X, 275 * PROP_Y, 0, to_string(wool).c_str());
}

void player1Viewer::viewLongestRoads()
{
	al_draw_text(fontForLongestRoad, al_color_name("black"), 74 * PROP_X, 469 * PROP_Y, 0, to_string(player->getLongestRoad()).c_str());
}
...
void player1Viewer::viewAmountOfCards()
{
	...
	if (amountOfCards > 9)
	{
		al_draw_text(fontForAmountOfCards, al_color_name("black"), 248 * PROP_X, 461 * PROP_Y, 0, to_string(amountOfCards).c_str());
	}
	else
	{

		al_draw_text(fontForVictoryPoints, al_color_name("black"), 252 * PROP_X, 461 * PROP_Y, 0, to_string(amountOfCards).c_str());
	}
}
