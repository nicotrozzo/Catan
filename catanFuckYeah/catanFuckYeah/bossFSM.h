...
#define TX(x) (static_cast<void (genericFSM::* )(genericEvent *)>(&bossFSM::x)) //casteo a funcion, por visual
	...
	{ { PLAYING,TX(sendInputEv) },			{ REMATCH,TX(doNothing/*NO VA ESTO*/) }, { START_MENU,TX(newStMn2) },	{ WAITING_TO_QUIT,TX(finishGame) },		  { PLAYING,TX(closeGame) },			  { PLAYING,TX(sendNetwEv) },					{ PLAYING,TX(sendTimerEvent) } },				//PLAYING
	{ { REMATCH,TX(sendInputEv) },			{ HANDSHAKING,TX(doNothing/*NO VA ESTO*/) }, { START_MENU,TX(newStMn2) },	{ WAITING_TO_QUIT,TX(sendGameOver) },{ REMATCH,TX(closeRematch) },			  { REMATCH,TX(parseNetwEv) },					{ REMATCH,TX(sendTimerEv) } },				//REMATCH
