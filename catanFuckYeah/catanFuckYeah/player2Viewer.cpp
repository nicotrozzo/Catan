...
void player2Viewer::viewAmountOfCards()
{
	...
	if (amountOfCards > 9)
	{
		al_draw_text(fontForAmountOfCards, al_color_name("black"), 247, 786, 0, to_string(amountOfCards).c_str());
	}
	else
	{
		al_draw_text(fontForVictoryPoints, al_color_name("black"), 252, 786, 0, to_string(amountOfCards).c_str());
	}
...
void player2Viewer::viewVictoryPoints()
{
	...
	al_draw_text(fontForVictoryPoints, al_color_name("black"), 148, 898, 0, points.c_str());
}
